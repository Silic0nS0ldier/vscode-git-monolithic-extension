...
export class Repository implements Disposable {
    ...
    private _onDidChangeOperations = onetime(() =>
        anyEvent(this.onRunOperation as Event<any>, this.onDidRunOperation as Event<any>)
    );
    ...
    constructor(
        ...
        const onConfigListener = filterEvent(
            workspace.onDidChangeConfiguration,
            e => e.affectsConfiguration("git.alwaysShowStagedChangesResourceGroup", root),
        );
    ...
    async getInputTemplate(): Promise<string> {
        const commitMessage =
            (await Promise.all([this.repository.getMergeMessage(), this.repository.getSquashMessage()])).find(msg =>
                !!msg
            );
    ...
    updateModelState = throat(1, async () => {
        ...
        const [refs, remotes, submodules, rebaseCommit] = await Promise.all([
        ...
        status.forEach(raw => {
            ...
            switch (raw.x + raw.y) {
                    ...
                    switch (untrackedChanges) {
                        case "mixed":
                            return workingTree.push(
                                new Resource(this, ResourceGroupType.WorkingTree, uri, Status.UNTRACKED, useIcons),
                            );
                        case "separate":
                            return untracked.push(
                                new Resource(this, ResourceGroupType.Untracked, uri, Status.UNTRACKED, useIcons),
                            );
                        default:
                            return undefined;
                    }
                case "!!":
                    switch (untrackedChanges) {
                        case "mixed":
                            return workingTree.push(
                                new Resource(this, ResourceGroupType.WorkingTree, uri, Status.IGNORED, useIcons),
                            );
                        case "separate":
                            return untracked.push(
                                new Resource(this, ResourceGroupType.Untracked, uri, Status.IGNORED, useIcons),
                            );
                        default:
                            return undefined;
                    }
                case "DD":
                    return merge.push(new Resource(this, ResourceGroupType.Merge, uri, Status.BOTH_DELETED, useIcons));
                case "AU":
                    return merge.push(new Resource(this, ResourceGroupType.Merge, uri, Status.ADDED_BY_US, useIcons));
                case "UD":
                    return merge.push(
                        new Resource(this, ResourceGroupType.Merge, uri, Status.DELETED_BY_THEM, useIcons),
                    );
                case "UA":
                    return merge.push(new Resource(this, ResourceGroupType.Merge, uri, Status.ADDED_BY_THEM, useIcons));
                case "DU":
                    return merge.push(new Resource(this, ResourceGroupType.Merge, uri, Status.DELETED_BY_US, useIcons));
                case "AA":
                    return merge.push(new Resource(this, ResourceGroupType.Merge, uri, Status.BOTH_ADDED, useIcons));
                case "UU":
                    return merge.push(new Resource(this, ResourceGroupType.Merge, uri, Status.BOTH_MODIFIED, useIcons));
            }

            switch (raw.x) {
                case "M":
                    index.push(new Resource(this, ResourceGroupType.Index, uri, Status.INDEX_MODIFIED, useIcons));
                    break;
                case "A":
                    index.push(new Resource(this, ResourceGroupType.Index, uri, Status.INDEX_ADDED, useIcons));
                    break;
                case "D":
                    index.push(new Resource(this, ResourceGroupType.Index, uri, Status.INDEX_DELETED, useIcons));
                    break;
                case "R":
                    index.push(
                        new Resource(this, ResourceGroupType.Index, uri, Status.INDEX_RENAMED, useIcons, renameUri),
                    );
                    break;
                case "C":
                    index.push(
                        new Resource(this, ResourceGroupType.Index, uri, Status.INDEX_COPIED, useIcons, renameUri),
                    );
                    break;
            }
            ...
            switch (raw.y) {
                case "M":
                    workingTree.push(
                        new Resource(this, ResourceGroupType.WorkingTree, uri, Status.MODIFIED, useIcons, renameUri),
                    );
                    break;
                case "D":
                    workingTree.push(
                        new Resource(this, ResourceGroupType.WorkingTree, uri, Status.DELETED, useIcons, renameUri),
                    );
                    break;
                case "A":
                    workingTree.push(
        ...
        switch (countBadge) {
            ...
            case "tracked":
                if (untrackedChanges === "mixed") {
                    count -= this.workingTreeGroup.resourceStates.filter(r =>
